...
class PCVROneTrans(EmbeddingParameterMixin, nn.Module):
    ...
    def _encode_non_sequence(self, inputs: PCVRModelInput) -> torch.Tensor:
        parts = [self.user_tokenizer(inputs.user.int_values, inputs.user.int_missing_mask)]
        ...
        parts.append(self.item_tokenizer(inputs.item.int_values, inputs.item.int_missing_mask))
    ...
    def _sequence_stats_token(self, inputs: PCVRModelInput) -> torch.Tensor | None:
        ...
        if not vectors:
            return inputs.user.int_values.new_zeros(batch_size, 1, self.d_model, dtype=torch.float32)
    ...
    def _encode_sequence_stream(self, inputs: PCVRModelInput) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if not pieces:
            batch_size = inputs.user.int_values.shape[0]
            device = inputs.user.int_values.device
            return torch.zeros(batch_size, 0, self.d_model, device=device), torch.zeros(batch_size, 0, dtype=torch.bool, device=device)
