...
def build_pcvr_experiment_cases(config_root: Path) -> tuple[ExperimentCase, ...]:
...
def discover_pcvr_experiment_cases() -> tuple[ExperimentCase, ...]:
    return build_pcvr_experiment_cases(CONFIG_ROOT)


def discover_nonbaseline_pcvr_experiment_paths() -> tuple[str, ...]:
    return tuple(case.path for case in discover_pcvr_experiment_cases() if case.path != "config/baseline")
