...
class SparseEmbeddingBagMean(torch.autograd.Function):
    ...
    def forward(ctx, weight: torch.Tensor, values: torch.Tensor) -> torch.Tensor:
        ...
        ctx.batch_idx = torch.arange(
            batch_size, device=values.device
        ).repeat_interleave(bag_size)
...
def sparse_embedding_bag_mean(
    weight: torch.Tensor, values: torch.Tensor
) -> torch.Tensor:
        ...
        *,
        ...
        for vocab_size, _offset, _length in self.feature_specs:
            if int(vocab_size) <= 0:
                ...
                and int(vocab_size) > emb_skip_threshold
                and self.compress_high_cardinality
            ):
                self._embedding_index.append(-1)
                self._compressed_embedding_index.append(len(self.compressed_embeddings))
                self.compressed_embeddings.append(
                    nn.Embedding(
                        int(emb_skip_threshold) + 1, emb_dim, padding_idx=0, sparse=True
                    )
                )
                ...
                self.embeddings.append(
                    nn.Embedding(
                        int(vocab_size) + 1, emb_dim, padding_idx=0, sparse=True
                    )
                )
    ...
    def forward(self, int_feats: torch.Tensor) -> torch.Tensor:
        ...
        for feature_index, (vocab_size, offset, length) in enumerate(
            ...
            if embedding_index >= 0:
                values = (
                    int_feats[:, offset : offset + length]
                    .to(torch.long)
                    .clamp(min=0, max=int(vocab_size))
                )
                tokens.append(
                    sparse_embedding_bag_mean(
                        self.embeddings[embedding_index].weight, values
                    )
                )
            elif compressed_index >= 0:
                embedding = self.compressed_embeddings[compressed_index]
                values = (
                    int_feats[:, offset : offset + length].to(torch.long).clamp(min=0)
                )
                compressed_values = hash_compress_ids(
                    values, embedding.num_embeddings - 1
                )
                tokens.append(
                    sparse_embedding_bag_mean(embedding.weight, compressed_values)
                )
            else:
                tokens.append(
                    int_feats.new_zeros(batch_size, self.emb_dim, dtype=torch.float32)
                )
...
class EmbeddingParameterMixin:
    def get_sparse_params(self) -> list[nn.Parameter]:
        ...
        return [
            ...
            if parameter.data_ptr() in sparse_ptrs
        ]
    ...
    def get_dense_params(self) -> list[nn.Parameter]:
        ...
        return [
            parameter
            for parameter in self.parameters()
            if parameter.data_ptr() not in sparse_ptrs
        ]
