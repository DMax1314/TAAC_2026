...
- OOM 时先降 batch size、序列长度、模型宽度、`num_workers` 和训练 shuffle buffer；step-random 训练默认使用 `buffer_batches=1`，不要再沿用旧的 `20`。
...
| 训练 OOM                 | batch size、序列长度、模型宽度、`num_workers`、`buffer_batches` |
