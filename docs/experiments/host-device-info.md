...
配置项在 `config/host_device_info/__init__.py` 的 `HostDeviceInfoConfig` 中定义。主要可调参数：
...
```bash
uv run taac-train --experiment config/host_device_info
```
...
- 运行器源码：`config/host_device_info/runner.py`
- 包入口：`config/host_device_info/__init__.py`
