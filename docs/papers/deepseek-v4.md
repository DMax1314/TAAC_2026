...
> Converted automatically from DeepSeek_V4.pdf with light cleanup. Figures were extracted to img/deepseek-v4/.
...
![](img/deepseek-v4/DeepSeek_V4.pdf-0006-00.png)
...
where $F_l$ denotes the $l$-th layer (e.g., an MoE layer), whose input and output both lie in $\mathbb{R}^d$. Note that the actual layer input $A_l X_l \in \mathbb{R}^d$ is also $d$-dimensional, so the expanded residual
...
$$
B_l \in \mathcal{M} := \left\{ M \in \mathbb{R}^{n \times n} \mid M \mathbf{1}_n = \mathbf{1}_n,\ \mathbf{1}_n^\top M = \mathbf{1}_n^\top,\ M \ge 0 \right\}
$$
...
**Dynamic Parameterization.** The parameters of the three linear mappings are dynamically generated and decomposed into a dynamic, input-dependent component and a static, input-independent component. Given the input _X_<sub>l</sub> ∈ **R**<sup>n<sub>hc</sub> × d</sup>, it is first flattened and normalized as \(\hat{X}_l = \operatorname{RMSNorm}(\operatorname{vec}(X_l))\in \mathbb{R}^{1 \times n_{\mathrm{hc}} d}\). We then generate the unconstrained raw parameters \(\widetilde{A}_l \in \mathbb{R}^{1 \times n_{\mathrm{hc}}}\), \(\widetilde{B}_l \in \mathbb{R}^{n_{\mathrm{hc}} \times n_{\mathrm{hc}}}\), and \(\widetilde{C}_l \in \mathbb{R}^{n_{\mathrm{hc}} \times 1}\):

$$
 \widetilde{A}_l = \alpha_l^{\mathrm{pre}} \cdot (\hat{X}_l W_l^{\mathrm{pre}}) + S_l^{\mathrm{pre}}
$$

$$
 \widetilde{B}_l = \alpha_l^{\mathrm{res}} \cdot \mathrm{Mat}(\hat{X}_l W_l^{\mathrm{res}}) + S_l^{\mathrm{res}}
$$

$$
 \widetilde{C}_l = \alpha_l^{\mathrm{post}} \cdot (\hat{X}_l W_l^{\mathrm{post}})^\top + S_l^{\mathrm{post}}
$$

where _W_<sub>l</sub><sup>pre</sup> and _W_<sub>l</sub><sup>post</sup> are learnable parameters for generating the dynamic components, while _W_<sub>l</sub><sup>res</sup> generates the residual mapping component. Mat(·) reshapes a vector of size 1 × _n_<sub>hc</sub><sup>2</sup> into a matrix of size _n_<sub>hc</sub> × _n_<sub>hc</sub>. The terms _S_<sub>l</sub><sup>pre</sup>, _S_<sub>l</sub><sup>post</sup>, and _S_<sub>l</sub><sup>res</sup> are learnable static biases, and _α_<sub>l</sub><sup>pre</sup>, _α_<sub>l</sub><sup>res</sup>, and _α_<sub>l</sub><sup>post</sup> are learnable gating factors initialized to small values.

**Applying Parameter Constraints.** After obtaining the unconstrained raw parameters $\widetilde{A}_l$, $\widetilde{B}_l$, and $\widetilde{C}_l$, we apply the constraints described earlier to improve numerical stability. Specifically, for the input and output mappings, we use a sigmoid function $\sigma(\cdot)$ to ensure non-negativity and boundedness:
...
As for the residual mapping $\widetilde{B}_l$, we project it onto the manifold of doubly stochastic matrices $\mathcal{M}$. This is achieved by the Sinkhorn-Knopp algorithm, which first applies an exponential function to $\widetilde{B}_l$ to ensure positivity, yielding $M^{(0)} = \exp(\widetilde{B}_l)$, and then iteratively performs column and row normalization:

$$
M^{(t)} = T_r\bigl(T_c(M^{(t-1)})\bigr)
$$

where T _r_ and T _c_ denote row and column normalization, respectively. This iteration converges to a constrained doubly stochastic matrix _B_<sub>l</sub> = _M_<sup>(t<sub>max</sub>)</sup>. We choose _t_<sub>max</sub> = 20 as a practical value.

![](img/deepseek-v4/DeepSeek_V4.pdf-0009-00.png)

Figure 3 | Core architecture of CSA. It compresses the number of KV entries by a factor of _m_ and then applies DeepSeek Sparse Attention for further acceleration. Additionally, a small set of sliding-window KV entries is combined with the selected compressed KV entries to enhance local fine-grained dependencies.
...
where $W^{aKV}$, $W^{bKV}$, $W^{aZ}$, and $W^{bZ}$ are trainable parameters in $\mathbb{R}^{d \times c}$. Next, every $m$ KV entries in $C^a$ and $C^b$ are compressed into one entry according to their compression weights and learnable positional biases $B^a, B^b \in \mathbb{R}^{m \times c}$. This produces compressed entries $C^{\mathrm{Comp}}$, where each compressed entry $C_i^{\mathrm{Comp}} \in \mathbb{R}^{c}$ is computed by
...
![](img/deepseek-v4/DeepSeek_V4.pdf-0015-04.png)

Figure 5 | Illustration of our EP scheme with related works. Comet (Zhang et al., 2025b) overlaps Dispatch with Linear-1, and Linear-2 with Combine, separately. Our EP scheme achieves a finer-grained overlap by splitting and scheduling experts into waves. The theoretical speedup is evaluated in the configuration of the DeepSeek-V4-Flash architecture.
...
embedding module, the prediction head module, and the weights of all RMSNorm modules. For AdamW, we set its hyper-parameters to _𝛽_ 1 = 0.9, _𝛽_ 2 = 0.95, _𝜀_ = 10^−20, and weight_decay = 0.1. For Muon, we set the momentum to 0.95 and the weight decay to 0.1, and rescale the RMS of each update matrix to 0.18 for reutilization of the AdamW learning rate. We train DeepSeek-V4-Flash on 32T tokens, and as in DeepSeek-V3, we also employ a batch-size scheduling strategy that increases the batch size (in tokens) from a small size to 75.5M and then keeps it at 75.5M during most of the training. The learning rate is linearly warmed up in the first 2000 steps, maintained at 2.7 × 10^−4 for most of the training, and finally decayed to 2.7 × 10^−5 following a cosine schedule near the end of training. The training starts with a sequence length of 4K, and we gradually extend the training sequence length to 16K, 64K, and 1M. As for the sparse-attention setup, we first warm up the model with dense attention for the first 1T tokens, and introduce sparse attention at the sequence length of 64K for the rest of training. When introducing attention sparsity, we first set a short stage to warm up the lightning indexer in CSA, and then train the model with sparse attention for most of the remaining training. For auxiliary-loss-free load balancing, we set the bias update speed to 0.001. For the balance loss, we set its weight to 0.0001 to avoid extreme imbalance within single sequences. The MTP loss weight is set to 0.3 for most of the training, and to 0.1 upon the start of learning-rate decay.

**DeepSeek-V4-Pro.** Except for specific hyper-parameter values, the training setup of DeepSeek-V4-Pro is largely consistent with that of DeepSeek-V4-Flash. We employ the Muon optimizer for the majority of parameters, but use the AdamW optimizer for the embedding module, the prediction head module, and the weights of all RMSNorm modules. The hyper-parameters of AdamW and Muon are the same as those of DeepSeek-V4-Flash. We train DeepSeek-V4-Pro on 33T tokens, and also employ a batch-size scheduling strategy, with the maximum batch size being 94.4M tokens. The learning-rate schedule is largely the same as that of DeepSeek-V4-Flash, but the peak learning rate is set to 2.0 × 10^−4 and the final learning rate is set to 2.0 × 10^−5. Training also starts with a sequence length of 4K, and the length is gradually extended to 16K, 64K, and 1M. Compared with DeepSeek-V4-Flash, DeepSeek-V4-Pro starts with a longer stage of dense attention, and the strategy for introducing sparse attention is the same two-stage method used in DeepSeek-V4-Flash. For auxiliary-loss-free load balancing, we set the bias update speed to 0.001. For the balance loss, we set its weight to 0.0001 to avoid extreme imbalance within single sequences. The MTP loss weight is set to 0.3 for most of the training, and to 0.1 upon the start of learning-rate decay.
...
| Architecture  | # Activated Params      | -           | 37B                    | 13B                        | 49B                      |
| Architecture  | # Total Params          | -           | 671B                   | 284B                       | 1.6T                     |
...
| ------------------ | ---------------------------------------------------------- | ----------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| **Non-think**      | Fast, intuitive responses based on habits or simple rules. | Routine daily tasks, emergency reactions, and low-risk decisions. | Summary only, without an explicit thinking block.                                              |
| **Think High**     | Conscious logical analysis; slower but more accurate.      | Complex problem solving, planning, and medium-risk decisions.     | `<think>` thinking tokens `</think>` summary                                                   |
| **Think Max**      | Pushes reasoning to its fullest extent; slow but powerful. | Exploring the boundary of the model's reasoning capability.       | A special system prompt at the beginning, followed by `<think>` thinking tokens and a summary. |
...
![](img/deepseek-v4/DeepSeek_V4.pdf-0032-00.png)
...
| `<extracted_url>` | Extracts each URL in the user prompt.                                                 | `...<User>{prompt}<extracted_url>`                 |
| `<read_url>`      | Determines whether each extracted URL should be fetched and read.                     | `<extracted_url>{url}<read_url>`                   |

#### 5.1.2. On-Policy Distillation

After training multiple domain-specific experts via specialized fine-tuning and reinforcement learning, we employ multi-teacher On-Policy Distillation (OPD) as the primary technique for merging expert capabilities into the final model. OPD has emerged as an effective post-training paradigm for efficiently transferring the knowledge and capabilities of domain experts to a single, unified model. This is achieved by having the student learn from the output distributions of teacher models on its own generated trajectories. Formally, given a set of _𝑁_ expert models { _𝜋𝐸_1, _𝜋𝐸_2, _. . ._, _𝜋𝐸𝑁_ }, the OPD objective function is defined as:

$$
\mathcal{L}_{\mathrm{OPD}}(\theta) = \sum_{i=1}^{N} w_i \cdot D_{\mathrm{KL}}\left(\pi_\theta \;\|\; \pi_{E_i}\right).
$$

In this formulation, _𝑤𝑖_ represents the assigned weight for each expert, typically determined by its relative importance. Computing the reverse KL loss _D_<sub>KL</sub>( _𝜋𝜃_ ∥ _𝜋𝐸𝑖_ ) requires sampling training trajectories from the student _𝜋𝜃_ to maintain on-policy learning. The underlying logic ensures that the unified policy _𝜋𝜃_ selectively learns from the specialized expert relevant to the current task context, such as aligning with the mathematics expert for math reasoning tasks and the coding expert for programming tasks. Through this mechanism, knowledge from physically distinct expert weights is consolidated into a unified parameter space via logits-level alignment, practically circumventing the performance degradation often encountered in traditional weight merging or mixed RL techniques. In this stage, more than ten teacher models covering various domains are employed to distill a single student model.

In handling the above OPD objective, prior works usually simplify the full-vocabulary KL loss into a token-level KL estimate at each token position, and then reuse the RL framework by treating a stop-gradient version of the per-token teacher-student log-probability gap as the advantage estimate in the policy loss calculation. Although this approach is resource-efficient, it leads to high variance in gradient estimation and often causes training instability. Therefore, we adopt full-vocabulary logit distillation in our OPD. Preserving the complete logit distribution when calculating the reverse KL loss yields more stable gradient estimates and ensures faithful distillation of the teachers’ knowledge. In the following subsection, we describe the engineering efforts that make full-vocabulary OPD feasible at scale.
...
**Fast Image Loading via Layered Storage.** DSec reconciles fast startup with a large and growing corpus of environment images through layered, on-demand loading. For containers, base images and filesystem commits are stored as 3FS-backed readonly EROFS layers mounted directly into overlay `lowerdir` s. We keep file metadata readily available on the local disk at mount time; meanwhile, data blocks are fetched from 3FS upon request. For microVMs, DSec uses the `overlaybd` (Li et al., 2020) disk format: the read-only base layer resides on 3FS for cross-instance sharing, while writes go to a local copy-on-write layer. Such snapshots are chainable, facilitating efficient versioning and millisecond-scale resumption.
...
| **Category**          | **Benchmark (Metric)**      | **Opus-4.6 Max** | **GPT-5.4 xHigh** | **Gemini-3.1-Pro High** | **K2.6 Thinking** | **GLM-5.1 Thinking** | **DeepSeek-V4-Pro Max** |
| --------------------- | --------------------------- | ---------------- | ----------------- | ----------------------- | ----------------- | -------------------- | ----------------------- |
...
| Long                  | CorpusQA 1M (ACC)           | **71.7**         | -                 | 53.8                    | -                 | -                    | 62.0                    |
...
| Agentic               | SWE Multilingual (Resolved) | **77.5**         | -                 | 80.6                    | 76.7              | 73.3                 | 76.2                    |
...
![](img/deepseek-v4/DeepSeek_V4.pdf-0040-02.png)

##### Frontier Regime

Putnam-2025 with hybrid formal-informal reasoning and substantial compute scaling.

![](img/deepseek-v4/DeepSeek_V4.pdf-0040-05.png)
...
![](img/deepseek-v4/DeepSeek_V4.pdf-0040-07.png)
...
![](img/deepseek-v4/DeepSeek_V4.pdf-0041-00.png)
...
![](img/deepseek-v4/DeepSeek_V4.pdf-0043-05.png)
...
| **Business Writing (`办公文本`)**  | **Subtotal (`小计`)**              | **1349** | **879**    | **436**     | **34**  | **65.16** | **32.32** | **2.52** |
...
| **Media Writing (`媒体文本`)**     | **Subtotal (`小计`)**              | **666**  | **386**    | **256**     | **24**  | **57.96** | **38.44** | **3.60** |
...
| **Everyday Writing (`生活文本`)**  | **Subtotal (`小计`)**              | **390**  | **271**    | **101**     | **18**  | **69.49** | **25.90** | **4.62** |
...
| **Oral Writing (`口头文本`)**      | **Subtotal (`小计`)**              | **319**  | **187**    | **120**     | **12**  | **58.62** | **37.62** | **3.76** |
| Official Document (`公文文本`)     | Administrative Doc (`事务文书`)    | 117      | 60         | 53          | 4       | 51.28     | 45.30     | 3.42     |
| Official Document (`公文文本`)     | Personal Doc (`个人文书`)          | 73       | 45         | 27          | 1       | 61.64     | 36.99     | 1.37     |
| Official Document (`公文文本`)     | Government Doc (`行政公文`)        | 34       | 19         | 14          | 1       | 55.88     | 41.18     | 2.94     |
| Official Document (`公文文本`)     | Speech (`发言稿`)                  | 3        | 1          | 2           | 0       | 33.33     | 66.67     | 0.00     |
| Official Document (`公文文本`)     | Essay Writing (`申论写作`)         | 3        | 1          | 1           | 1       | 33.33     | 33.33     | 33.33    |
| **Official Document (`公文文本`)** | **Subtotal (`小计`)**              | **230**  | **126**    | **97**      | **7**   | **54.78** | **42.17** | **3.04** |
...
| **Academic Writing (`学术文本`)**  | **Subtotal (`小计`)**              | **216**  | **137**    | **71**      | **8**   | **63.43** | **32.87** | **3.70** |
...
| **Subcategory (`文体`)**         | **#**    | **IF DS win** | **IF Gem win** | **IF Tie** | **IF DS%** | **IF Gem%** | **IF Tie%** | **WQ DS win** | **WQ Gem win** | **WQ Tie** | **WQ DS%** | **WQ Gem%** | **WQ Tie%** |
| -------------------------------- | -------- | ------------- | -------------- | ---------- | ---------- | ----------- | ----------- | ------------- | -------------- | ---------- | ---------- | ----------- | ----------- |
...
| **Total**                        | **2837** | **1703**      | **1119**       | **15**     | **60.03**  | **39.44**   | **0.53**    | **2198**      | **634**        | **5**      | **77.48**  | **22.35**   | **0.18**    |
...
| **Total**                                | **196** | **90** | **102**  | **4**   | **45.9%** | **52.0%** | **2.0%** |
